...
#         types_dictionary = {}
#         # for row in column_types_raw:
#         #     types_dictionary[row[1]] = row[2].lower()

#         # print(column_types_raw)
#         # column_types = self.connection.execute(f"SELECT * FROM {table_name} LIMIT 0").description
#         # self.connection.execute(f"SELECT * FROM ({query}) LIMIT 0").description
#         # Prepare the column expressions
#         column_expressions = []
#         for col in column_types_raw:
#             col_name, col_type = col[1], col[2].lower()
#             print(f"col_name: {col_name}, col_type: {col_type}")
#             if 'decimal' in col_type or 'numeric' in col_type:
#                 # For decimal types, we'll keep them as is and handle the conversion later
#                 column_expressions.append(col_name)
#             elif any(int_type in col_type for int_type in ['int', 'integer', 'bigint', 'smallint', 'tinyint']):
#                 column_expressions.append(f"CAST({col_name} AS VARCHAR) AS {col_name}")
#             elif 'binary' in col_type or 'varbinary' in col_type:
#                 column_expressions.append(f"LOWER(HEX({col_name})) AS {col_name}")
...
#             else:
#                 column_expressions.append(col_name)
#         subquery = f"SELECT * FROM {table_name} order by 1"
#         # Construct and execute the modified query
#         modified_query = f"SELECT {', '.join(column_expressions)} FROM ({subquery}) subquery"
#         print(modified_query)
#         df = self.get_data_as_df(modified_query)
