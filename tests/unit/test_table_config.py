...
def test_invalid_cdc_type(tmp_path, mock_config):
    ...
    with patch('os.path.join', return_value=str(csv_path)):
        ...
        assert str(exc_info.value) == "INVALID_TYPE is not a valid CDC type.  Please provide STANDARD_STREAM, APPEND_ONLY_STREAM, or leave it blank to default to STANDARD_STREAM."
